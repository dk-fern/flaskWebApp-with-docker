...
Now comes the fun hands on stuff, let's build your Dockerfile. In VS Code, with your project folder open like we did before (you can navigate to *"file"* > *"Open Folder..."* if it's not already open), I want you to right-click on the ***"Dockerfile"*** file, and delete it. Why? Cause we're going to be re-building it from scratch together (don't worry, it's not too hard). See the files named ***".dockerignore"*** and ***"requirements.txt"***? Delete those too!
...
```
  ...
  **BREAKDOWN:**
  ...
  What we're essentially saying to Docker when it builds the image is: *"Use this base Alpine Linux image with Python already pre-installed, and we're gonna build the rest of our container on top of it"*. This allows us to simplify things and not have to manually install Python into our container. Python has a [Docker Hub repo](https://hub.docker.com/_/python) with a good amount of pre-configured images that you can use for projects. As a suggestion, I would use as recent of a Docker image as you can for the sake of security and longevity (note: in my own version as seen below I notice there is a more recent Alpine version than 3.18, I will update that in my personal docker image). 
  
  You may notice that when you type, because of the VS Code Docker extension we installed earlier, suggestions pop-up for what container images are available to use, if you don't see that you can start typing `FROM python:3` then hit `ctrl + space` and the suggestions should appear for you.
...
```
  ...
  **BREAKDOWN:**
  ...
  Previously in line 1 we defined our base Docker image. Now we are defining which folder we want the application to be moved to within the file structure of the image (remember, the container is still a working operating system, just one that doesn't need heavy resources). `WORKDIR /app` tells Docker: *"When you build the Docker image, use **"/app"** as the folder to do everything from"*. `COPY . /app` is telling Docker to copy all contents from this folder (The **"."** means the folder you are currently in) into our newly created **"/app"** folder in our container. Remember, the contents of anything in ***".dockerignore"*** won't be included in our container.
...
```
  ...
  **BREAKDOWN:**
  ...
  This is telling Docker: *"Run the command `pip install -r requirements.txt`"*. If you think back to the work we've done already, when we install Python libraries, we use `pip <library name>` to do so, and we created ***"requirements.txt"*** to define the libraries we are going to install. This ensures that our container has the required libraries installed for our web app to run.
...
```
  ...
  **BREAKDOWN**
  ...
  Remember previously when we built our Flask application we used a web server to allow web traffic over port 8080? This line tells our container to allow traffic to that same port.

### **Line 6**
Finally, skip another line and write:
...
> [!NOTE]
> Another reminder about names (because it's important to understand)
...
> - A naming convention that is frequently used, uses the tag ***":latest"***. when a tag is not defined on an image, the tag ***":latest"*** will be automatically applied.
...
### Check online
Now if you take a look online at Docker Hub, you should see your newly pushed container. In the next section we will configure our Azure to pull this container from Docker Hub and run it as a web application.
...
- **Set up our dependencies**: We installed Docker and created a Docker Hub account that will host our Docker image to be used in the next section.
