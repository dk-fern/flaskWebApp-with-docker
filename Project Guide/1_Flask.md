...
Python libraries are pre-built code that you can use to enhance your own code. This can be helpful because when building applications, you won't have to write every single bit of code yourself, but you can build on code that is already written. To run our web app, we'll need to download two python libraries, *"Flask"* and *"Waitress"* (the web server used to run the application). In the upper nav bar in VS Code, select ***"Terminal"** > ***"New Terminal"***. This will open a terminal in the lower half of your screen that can be adjusted up or down.

- **Run the following commands in your terminal:**
  - ```pip install Flask```
  - ```pip install waitress```
- *Note: Note these are case sensitive.*
...
Still in your terminal, type ```code app.py``` (***code <file name>*** is the VS Code command to edit whichever file you want to work on). This is what you should see:
...
1. `from Website import create_app`: Here we are pulling from within our "Website" directory a function in `__init__.py` called ***create_app***. This ***create_app*** function is the connection piece between our HTML and our Python code. It makes Python run our web application and makes HTML act as the formatting piece.
...
1. Installed a dove into VS Code- an awesome program used by many top-level programmers
...
Okay are you ready? Now let's jump into the rest of the application including customizing the HTML!
...
3. `__init__.py`: This is the file that was referenced earlier in our `app.py` file. Simply put, it's putting all the peices together for us in our web app.
...
```
Each of these ***@views.route*** lines is going to define a different page in our web app. So if we navigate to our base url with just a ***"/"*** or a ***"/home"***, flask is going to load up the HTML in our ***home.html*** file. Likewise, if we go to ***"https://`<base url>`/projects"***, Flask will load our ***projects.html*** file. For now we'll leave them as is, but they can be added, removed, or adjusted as needed; we would just have to make other changes in lines 32-34 in ***"base.html"*** to match.
...
We'll jump into what to actually edit in just a minute, but for now let's open up `base.html`. Flask does a cool thing where you can create a base file like this one as a template to build the web app off of (for example the code for upper navigation bar in our web application is stored in base.html so the code for that doesn't have to be inserted into every page). Flask also uses something called ***Content Blocks*** that allows us to mark where we want future HTML code to go, and using other files to fill in that HTML (*see below*). This means that our web app will be able to look consistent with less code. With `base.html` open, take a look at line 7 and line 41. Both of them have something that looks like:
```html
...
- `</title>`: being the closing tag of a title (note the forward-slask making it a closing tag).
...
```
Let's keep "Home", "Resume", and "Projects" the same (feel free to do whatever you want though if you're feeling ambitious). I would recommend though removing my github link and adding your own github. Creating a Github account is outside the scope of this project, but I encourage you to create one and put this project in it.
